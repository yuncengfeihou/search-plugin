...
function searchMessages(keyword) {
    ...
    if (settings.searchScope === "loaded") {
        ...
        getChatLogLength(context.chatId).then(length => {
            fetchChatLog(context.chatId, 0, length).then(fullChat => {
                results = fullChat.filter((msg, index) => msg.content.toLowerCase().includes(keyword.toLowerCase())).map(msg => ({
                    id: index,
                    content: msg.content
                }));
                if (results.length > 0) {
        ...
        }).catch(() => toastr.error("关键词检索失败：无法获取全文记录"));
    }
}
